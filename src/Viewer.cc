...
void Viewer::newParameterLoader(Settings *settings) {
    ...
    mFrameViewerZoom = settings->frameViewerZoom();
...
bool Viewer::ParseViewerParamFile(cv::FileStorage &fSettings)
{
    ...
    node = fSettings["Viewer.FrameViewerZoom"];
    if(!node.empty())
    {
        mFrameViewerZoom = node.real();
    }
    else
    {

        std::cerr << "*Viewer.FrameViewerZoom parameter doesn't exist or is not a real number, default value is 1.0*" << std::endl;
        mFrameViewerZoom = 1.0;
    }
...
void Viewer::Run()
{
    ...
    pangolin::Var<bool> menuImgColor("menu.Show color", true, true);
    ...
    while(1)
    {
        ...
        cv::Mat toShow;
        cv::Mat im = mpFrameDrawer->DrawFrame(trackedImageScale, menuImgColor);

        if(both){
            cv::Mat imRight = mpFrameDrawer->DrawRightFrame(trackedImageScale, menuImgColor);
        ...
        if(mFrameViewerZoom != 1.f)
        {
            int width = toShow.cols * mFrameViewerZoom;
            int height = toShow.rows * mFrameViewerZoom;
