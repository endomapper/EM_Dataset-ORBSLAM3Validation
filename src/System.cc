...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false), mnNumProcessedFrames(0)
{
...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename, double time_video, int frame_video)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageStereo(imLeftToFeed,imRightToFeed,timestamp,filename, time_video, frame_video);
...
Sophus::SE3f System::TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename, double time_video, int frame_video)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(imToFeed,imDepthToFeed,timestamp,filename, time_video, frame_video);
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename, double time_video, int frame_video)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,filename, time_video, frame_video);
...
void System::SaveAtlasStats(std::string &filename)
{

    ofstream f_txt;
    string file_txt = filename + ".txt";
    f_txt.open(file_txt.c_str());
    f_txt << fixed;
    ofstream f_csv;
    string file_csv = filename + ".csv";
    f_csv.open(file_csv.c_str());
    f_csv << fixed;

    std::vector<Map*> vpMaps = mpAtlas->GetAllMaps();
    int numMaps = vpMaps.size();
    if(vpMaps[numMaps-1]->GetAllKeyFrames().size() == 0)
    {
        // Last map is empty
        numMaps -= 1;
    }
    f_txt << "Atlas Stats" << endl;
    f_txt << "-----------" << endl << endl;
    f_txt << "Number of Maps: " << numMaps << endl;
//    cout << "Atlas Stats" << endl;
//    cout << "-----------" << endl << endl;
//    cout << "Number of Maps: " << vpMaps.size() << endl << endl;

    f_csv << "Number of processed Frames," << mnNumProcessedFrames << endl;
    f_csv << "Number of Maps," << numMaps << endl << endl;
    f_csv << "Map_id,Number KFs,Number of MPs,Frames located,Number of reloc,Number of Loops,Number of Merges,Initial Frame,Last Frame" << endl;
    ...
    for (int i = 0; i < vpMaps.size(); ++i) {
        ...
        int id = pMapi->GetId();
        int numKFs = pMapi->GetAllKeyFrames().size();
        if(numKFs == 0)
        {
            continue;
        }
        int numMPs = pMapi->GetAllMapPoints().size();
        int numLocated = pMapi->GetNumFramesLocated();
        int numReloc = pMapi->GetNumFramesRelocalizated();
        int numLoops = pMapi->GetNumFramesLoop();
        int numMerges = pMapi->GetNumFramesMerge();
        string nameInitialF = pMapi->GetNameInitialFrame();
        string nameLastF = pMapi->GetNameLastFrame();

        f_txt << "---" << endl;
        f_txt << "Map id: " << id << endl;
        f_txt << "  - Number of KeyFrames: " << numKFs << endl;
        f_txt << "  - Number of MapPoints: " << numMPs << endl;
        f_txt << "  - Number of located Frames: " << numLocated << endl;
        f_txt << "  - Number of relocalizations: " << numReloc << endl;
        f_txt << "  - Number of loops: " << numLoops << endl;
        f_txt << "  - Number of merges: " << numMerges << endl;
        f_txt << "  - Name of initial Frame: " << nameInitialF << endl;
        f_txt << "  - Name of last Frame: " << nameLastF << endl << endl;

//        cout << "---" << endl;
//        cout << "Map id: " << id << endl;
//        cout << "  - Number of KeyFrames: " << numKFs << endl;
//        cout << "  - Number of MapPoints: " << numMPs << endl;
//        cout << "  - Number of located Frames: " << numLocated << endl;
//        cout << "  - Number of relocalizations: " << numReloc << endl;
//        cout << "  - Number of loops: " << numLoops << endl;
//        cout << "  - Number of merges: " << numMerges << endl;
//        cout << "  - Name of initial Frame: " << nameInitialF << endl;
//        cout << "  - Name of last Frame: " << nameLastF << endl << endl;

        f_csv << id << "," << numKFs << "," << numMPs << "," << numLocated << "," << numReloc << "," << numLoops << "," << numMerges << "," << nameInitialF << "," << nameLastF << endl;

        totalKFs += numKFs;
        totalMPs += numMPs;
        totalLocated += numLocated;
        totalReloc += numReloc;
        totalLoops += numLoops;
    ...
    f_txt << endl << "-------" << endl;
    f_txt << "Total stats on all maps: " << endl;
    f_txt << "  - Number of processed Frames: " << mnNumProcessedFrames << endl;
    f_txt << "  - Number of KeyFrames: " << totalKFs << endl;
    f_txt << "  - Number of MapPoints: " << totalMPs << endl;
    f_txt << "  - Number of located Frames: " << totalLocated << endl;
    f_txt << "  - Number of relocalizations: " << totalReloc << endl;
    f_txt << "  - Number of loops: " << totalLoops << endl;
    f_txt << "  - Number of merges: " << totalMerges << endl;
