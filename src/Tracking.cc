...
void Tracking::newParameterLoader(Settings *settings) {
    ...
    mbRGB = settings->rgb();
    mImageChannelColor = settings->channelToExtractFeatures();
    ...
    float fkFactor = settings->kFactor();

    mpORBextractorLeft = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,fkFactor);

    if(mSensor==System::STEREO || mSensor==System::IMU_STEREO)
        mpORBextractorRight = new ORBextractor(nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,fkFactor);

    if(mSensor==System::MONOCULAR || mSensor==System::IMU_MONOCULAR)
        mpIniORBextractor = new ORBextractor(5*nFeatures,fScaleFactor,nLevels,fIniThFAST,fMinThFAST,fkFactor);
...
bool Tracking::ParseCamParamFile(cv::FileStorage &fSettings)
{
    ...
    mImageChannelColor = 0;
    int nChannelColor = fSettings["Camera.channel2features"];
    if(nChannelColor >= 0 && nChannelColor <= 3)
    {
        //Valid channel color
        mImageChannelColor = nChannelColor;
    }
    cout << "- channel color to extract features(ignored if grayscale): ";
    switch (mImageChannelColor) {
    case 1:
        cout << "Blue" << endl;
        break;
    case 2:
...
bool Tracking::ParseORBParamFile(cv::FileStorage &fSettings)
{
    ...
    node = fSettings["Optimization.kFactor"];
    if(!node.empty() && node.isReal())
    {
        fkFactor = node.real();
    }
    ...
    {
        std::cerr << "*Optimization.kFactor parameter doesn't exist or is not a real value, default value is 1.0*" << std::endl;
    }
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename, double time_video, int frame_video)
{
    //cout << "GrabImageStereo" << endl;

    mImGray = imRectLeft.clone();
    mImBGR = imRectLeft.clone();
    cv::Mat imGrayRight = imRectRight;
    mImRight = imRectRight;
    mImRightBGR = imRectRight.clone();
    ...
    if(mImGray.channels()==3)
    {
        ...
        if(mbRGB)
        {
            cvtColor(mImGray,mImBGR,cv::COLOR_RGB2BGR);
            //cvtColor(mImGray,mImGray,cv::COLOR_RGB2GRAY);
            cvtColor(imGrayRight,mImRightBGR,cv::COLOR_RGB2BGR);
            //cvtColor(imGrayRight,imGrayRight,cv::COLOR_RGB2GRAY);
        }
//        else
//        {
//            mImBGR = mImGray.clone();
//            cvtColor(mImGray,mImGray,cv::COLOR_BGR2GRAY);
//            mImRightBGR = imGrayRight.clone();
...
//        }
    ...
    else if(mImGray.channels()==4)
    {
        ...
        if(mbRGB)
        {
            cvtColor(mImGray,mImBGR,cv::COLOR_RGBA2BGR);
            //cvtColor(mImGray,mImGray,cv::COLOR_RGBA2GRAY);
            cvtColor(imGrayRight,mImRightBGR,cv::COLOR_RGBA2BGR);
            //cvtColor(imGrayRight,imGrayRight,cv::COLOR_RGBA2GRAY);
        }
        else
        {
            cvtColor(mImGray,mImBGR,cv::COLOR_BGRA2BGR);
            //cvtColor(mImGray,mImGray,cv::COLOR_BGRA2GRAY);
            cvtColor(imGrayRight,mImRightBGR,cv::COLOR_BGRA2BGR);
            //cvtColor(imGrayRight,imGrayRight,cv::COLOR_BGRA2GRAY);
        }
    }

    if(mImBGR.channels() >= 3)
    {
        switch (mImageChannelColor) {
        case 1: // Blue
            extractChannel(mImBGR, mImGray, 0);
            extractChannel(mImRightBGR,mImRight, 0);
            break;
        case 2: // Green
            extractChannel(mImBGR, mImGray, 1);
            extractChannel(mImRightBGR,mImRight, 1);
            break;
        case 3: // Red
            extractChannel(mImBGR, mImGray, 2);
            extractChannel(mImRightBGR,mImRight, 2);
            break;
        default: // GrayScale
            cvtColor(mImBGR,mImGray,cv::COLOR_BGR2GRAY);
            cvtColor(mImRightBGR,mImRight,cv::COLOR_BGR2GRAY);
            break;
        }
    }

    mbHasFrameInfo = false;
    if(time_video >= 0 && frame_video >= 0)
    {
        mbHasFrameInfo = true;
        mnNumVideoFrame = frame_video;
        mTimeVideoFrame = time_video;
    }
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename, double time_video, int frame_video)
{
    mImGray = imRGB.clone();
    mImBGR = imRGB.clone();
    cv::Mat imDepth = imD;
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename, double time_video, int frame_video)
{
    //mImGray = im.clone();
    mImBGR = im.clone();
    if(mImBGR.channels()==3)
    {
        if(mbRGB)
        {
            cvtColor(mImBGR,mImBGR,cv::COLOR_RGB2BGR);
            //cvtColor(mImBGR,mImGray,cv::COLOR_BGR2GRAY);
        }
//        else
//        {
...
//        }
    ...
    else if(mImBGR.channels()==4)
    {
        if(mbRGB)
        {
            cvtColor(mImBGR,mImBGR,cv::COLOR_RGBA2BGR);
            //cvtColor(mImBGR,mImGray,cv::COLOR_BGR2GRAY);
        }
        else
        {
            cvtColor(mImBGR,mImBGR,cv::COLOR_BGRA2BGR);
            //cvtColor(mImBGR,mImGray,cv::COLOR_BGR2GRAY);
        }
    }
...
void Tracking::Track()
{
    ...
    else
    {
            ...
            else
            {
                ...
                if (mState == RECENTLY_LOST)
                {
                    ...
                    else
                    {
                        ...
                        if(bOK)
                        {
                            mpAtlas->GetCurrentMap()->CountFrameRelocalizated();
                        }
                    }
                }
                else if (mState == LOST)
        ...
        if(bOK)
        {
            mpAtlas->GetCurrentMap()->CountFrameLocated();
            mState = OK;
        }
        else if (mState == OK)
        {
