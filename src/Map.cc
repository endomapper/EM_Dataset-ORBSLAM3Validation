...
mbFail(false), mIsInUse(false), mHasTumbnail(false), mbBad(false), mnMapChangeNotified(0), mbIsInertial(false), mbIMU_BA1(false), mbIMU_BA2(false)
{
    ...
    mstrInitialFrameName = "";
    mstrLastFrameName = "";
    mnNumLocatedFrames = 0;
    mnNumRelocalizations = 0;
    mnNumLoops = 0;
    mnNumMerges = 0;
}
...
void Map::AddKeyFrame(KeyFrame *pKF)
{
    ...
    if(mspKeyFrames.empty()){
        ...
        mpKFinitial = pKF;
        mpKFlowerID = pKF;
        mstrInitialFrameName = pKF->mNameFile;
    }
    mstrLastFrameName = pKF->mNameFile;
...
void Map::CountFrameLocated()
{
    mnNumLocatedFrames++;
}

void Map::CountFrameRelocalizated()
{
    mnNumRelocalizations++;
}

void Map::CountFrameLoop()
{
    mnNumLoops++;
}

void Map::CountFrameMerge()
{
    mnNumMerges++;
}

std::string Map::GetNameInitialFrame()
{
    return mstrInitialFrameName;
}

std::string Map::GetNameLastFrame()
{
    return mstrLastFrameName;
}

unsigned int Map::GetNumFramesLocated()
{
    return mnNumLocatedFrames;
}

unsigned int Map::GetNumFramesRelocalizated()
{
    return mnNumRelocalizations;
}

unsigned int Map::GetNumFramesLoop()
{
    return mnNumLoops;
}

unsigned int Map::GetNumFramesMerge()
{
    return mnNumMerges;
}
