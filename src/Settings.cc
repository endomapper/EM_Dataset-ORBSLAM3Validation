...
namespace ORB_SLAM3 {
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        channel2ExtractFeat_ = readParameter<int>(fSettings,"Camera.channel2features",found,false);
    }
    ...
    void Settings::readORB(cv::FileStorage &fSettings) {
        ...
        kFactor_ = readParameter<float>(fSettings,"Optimization.kFactor",found, false);
        if(!found)
            kFactor_ = 1.0f;
    }
    ...
    void Settings::readViewer(cv::FileStorage &fSettings) {
        ...
        frameViewerZoom_ = readParameter<float>(fSettings,"Viewer.FrameViewerZoom",found,false);
        if(!found)
            frameViewerZoom_ = 1.0f;
    }
    ...
    ostream &operator<<(std::ostream& output, const Settings& settings){
        ...
        output << "\t-Image channel to extract features: ";
        switch (settings.channel2ExtractFeat_) {
        case 1:
            cout << "Blue" << endl;
            break;
        case 2:
        ...
        output << "\t-Factor k: " << settings.kFactor_ << endl;
