...
cv::Mat FrameDrawer::DrawFrame(float imageScale, bool bShowInColor)
{
    cv::Mat im;
    ...
    //Copy variables within scoped mutex
    {
        ...
        if(im.empty())
        {
            cout << "Waiting to an image in FrameDrawer..." << endl;
    ...
    {
        im = imWithInfo.clone();
        DrawFrameInfo(im, imWithInfo);
    }

    return imWithInfo;
}

cv::Mat FrameDrawer::DrawRightFrame(float imageScale, bool bShowInColor)
{
    cv::Mat im;
    vector<cv::KeyPoint> vIniKeys; // Initialization: KeyPoints in reference frame
    vector<int> vMatches; // Initialization: correspondeces with reference keypoints
...
void FrameDrawer::DrawFrameInfo(cv::Mat &im, cv::Mat &imText)
{
    int sec = ((int) (mTimeVideoFrame / 1e3)); // from ms to seconds
    int hour = sec / 3600;
    sec = sec - (hour * 3600);
    int min = sec / 60;
    sec = sec - (min * 60);
    ...
    stringstream s;
    s << " Video frame: " << mnVideoFrame;
    s << " | Video time: " << std::setw(2) << std::setfill('0') << hour << ":"
      << std::setw(2) << std::setfill('0') << min << ":"
      << std::setw(2) << std::setfill('0') << sec;

    //std::cout << "Text info: " << s.str() << std::endl;
    ...
    cv::putText(imText,s.str(),cv::Point(5,(5 + textSize.height)),cv::FONT_HERSHEY_PLAIN,1,cv::Scalar(255,255,255),1,8);
}
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    if(both){
        ...
        pTracker->mImRightBGR.copyTo(mImRightBGR);
    ...
    mbHasVideoInfo = pTracker->mbHasFrameInfo;
    if(mbHasVideoInfo)
    {
        mnVideoFrame = pTracker->mnNumVideoFrame;
        mTimeVideoFrame = pTracker->mTimeVideoFrame;
    }
